...
try:
    from scripts.build_reward_sweep_manifest import PROFILE_TCV_DELTA_JDOT
    ...
    from build_reward_sweep_manifest import PROFILE_TCV_DELTA_JDOT
    from submit_two_pass_reward_sweep import submit_chain


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Submit a 60+12 delta-Jdot TCV-derivative reward sweep.")
    parser.add_argument(
        "--pass1-job",
        type=Path,
        default=Path("jobs/sweep_t15_csv_segmented_profile_rewards_12gpu_tcv_delta_pass1.sbatch"),
    )
    parser.add_argument(
        "--pass1-aggregate-job",
        type=Path,
    ...
    parser.add_argument(
        "--pass2-job",
        type=Path,
        default=Path("jobs/sweep_t15_csv_segmented_profile_rewards_12gpu_tcv_delta_pass2.sbatch"),
    )
    parser.add_argument(
        "--final-aggregate-job",
        type=Path,
    ...
    parser.add_argument("--root-prefix", default="outputs/t15_reward_sweep72_tcv_delta_jdot_2m5m")
