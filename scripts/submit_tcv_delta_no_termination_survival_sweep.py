...
try:
    from scripts.build_reward_sweep_manifest import PROFILE_TCV_DELTA_NO_TERMINATION_SURVIVAL, build_manifest
except ModuleNotFoundError:  # pragma: no cover - used when run as python3 scripts/...
    from build_reward_sweep_manifest import PROFILE_TCV_DELTA_NO_TERMINATION_SURVIVAL, build_manifest
...
def _write_manifest(root: str) -> str:
    ...
    manifest = build_manifest(
        "broad",
        profile=PROFILE_TCV_DELTA_NO_TERMINATION_SURVIVAL,
        runs_per_array_task=3,
        array_task_count=12,
    )
    if int(manifest["variant_count"]) != 36:
        raise RuntimeError(f"survival no-termination manifest must contain 36 variants, got {manifest['variant_count']}")
    folders = [str(variant["folder"]) for variant in manifest["variants"]]
    if folders[0] != "s000_s0_i0_a0" or folders[-1] != "s035_s2_i2_a3":
        raise RuntimeError(f"unexpected survival no-termination variant folder range: {folders[0]} ... {folders[-1]}")
    if {variant["sim"]["terminate_on_boundary_loss"] for variant in manifest["variants"]} != {False}:
        raise RuntimeError("survival no-termination variants must disable boundary termination")
    if {variant["sim"]["terminate_on_current_limit"] for variant in manifest["variants"]} != {False}:
        raise RuntimeError("survival no-termination variants must disable current termination")
    if {variant["reward"]["boundary_missing_weight"] for variant in manifest["variants"]} != {40.0, 60.0, 100.0}:
        raise RuntimeError("survival no-termination variants must vary boundary_missing_weight over {40,60,100}")
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    return str(manifest_path)


def submit_no_termination_survival_sweep(
    *,
    sweep_job: Path,
    aggregate_job: Path,
    root_prefix: str,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Submit a 36-run survival no-termination TCV delta-Jdot reward sweep.")
    parser.add_argument(
        "--sweep-job",
        type=Path,
        default=Path("jobs/sweep_t15_csv_segmented_profile_rewards_12gpu_tcv_delta_noterm_survival_onepass.sbatch"),
    )
    parser.add_argument(
        "--aggregate-job",
        type=Path,
    ...
    parser.add_argument("--root-prefix", default="outputs/t15_reward_sweep36_tcv_delta_noterm_survival_2m")
    args = parser.parse_args(argv)
    payload = submit_no_termination_survival_sweep(
