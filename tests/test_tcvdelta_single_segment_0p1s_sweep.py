...
BASE_CONFIG = ROOT / "configs/experiments/t15_csv_initial_single_segment_0p1s_static_boundary_mpo.yaml"


def test_tcvdelta_single_segment_manifest_has_36_mapped_variants() -> None:
...
def test_tcvdelta_single_segment_candidate_config_loads_with_100_step_invariants(tmp_path: Path) -> None:
    ...
    assert cfg.reference.duration_s == 0.1
    ...
    assert cfg.training.steps == 1_000_000
    assert cfg.training.num_envs == 256
    assert cfg.training.save_checkpoints is False


def test_tcvdelta_single_segment_aggregator_handles_success_and_failure(tmp_path: Path) -> None:
    root = tmp_path / "sweep"
...
def test_tcvdelta_single_segment_jobs_pass_bash_syntax() -> None:
    jobs = [
        ROOT / "jobs/sweep_t15_csv_single_segment_0p1s_static_boundary_tcvdelta_12gpu_36x1m.sbatch",
        ROOT / "jobs/aggregate_tcvdelta_single_segment_0p1s_reward_sweep.sbatch",
    ]
    for job in jobs:
        subprocess.run(["bash", "-n", str(job)], check=True)


def test_tcvdelta_single_segment_sweep_job_uses_100_step_local_replay_gpu_path() -> None:
    job = ROOT / "jobs/sweep_t15_csv_single_segment_0p1s_static_boundary_tcvdelta_12gpu_36x1m.sbatch"
    text = job.read_text(encoding="utf-8")
    assert "TRAIN_ENV_STEPS=${TRAIN_ENV_STEPS:-1000000}" in text
    assert "--eval-max-steps 100" in text
